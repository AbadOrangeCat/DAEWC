...
MAX_NUM_WORDS = 5000
MAXLEN = 1000
TOKENIZER_MODE = "union"  # "source" (paper-strict) OR "union" (source_train + target_train texts)
...
# Outputs
OUT_RAW_CSV = "results_daeewc_paperstyle_raw.csv"
OUT_SUMMARY_CSV = "results_daeewc_paperstyle_summary.csv"
...
def split_train_dev_test(
    ...
    X_train, X_tmp, y_train, y_tmp = train_test_split(
        X, y, test_size=(test_size + dev_size), random_state=seed, stratify=y
    )
    # dev fraction within tmp
    dev_frac_within_tmp = dev_size / (test_size + dev_size)
    X_dev, X_test, y_dev, y_test = train_test_split(
        X_tmp, y_tmp, test_size=(1.0 - dev_frac_within_tmp), random_state=seed, stratify=y_tmp
    )
    return list(X_train), list(X_dev), list(X_test), np.array(y_train), np.array(y_dev), np.array(y_test)
...
def sample_k_shot_per_class(
    ...
    if len(idx0) < k or len(idx1) < k:
        raise ValueError(f"Not enough samples for k-shot. Need k={k}, got n0={len(idx0)}, n1={len(idx1)}")
    sel0 = rng.choice(idx0, size=k, replace=False)
    sel1 = rng.choice(idx1, size=k, replace=False)
    sel = np.concatenate([sel0, sel1])
    rng.shuffle(sel)
    X_sub = [X_texts[i] for i in sel]
    y_sub = y[sel]
    return X_sub, y_sub
...
for seed in SEEDS:
    ...
    for shot_key in shot_keys:
        ...
        if INCLUDE_DAEWC and USE_EWC:
            ...
            set_trainable_daeewc_stage2(base, unfreeze_backbone=("shared_fc",))
            wrapper = CLWrapper(
                base_model=base,
