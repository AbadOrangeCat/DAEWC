...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        for shot in SHOTS:
            ...
            # The idea: make DAEWC a strict superset of the baselines, then pick the best setting on tgt-dev.
            if shot <= 20:
                candidates = [
                    # name, level, lr, tau, lambda_u, lambda_src, ewc_lambda, l2_anchor
                    ("A_adapter", "adapter", 2e-3, 0.95, 2.0, 0.00, 0.0, 0.0),
                    ("B_top+EWC", "top",     1e-3, 0.90, 2.0, 0.30, 5.0, 1e-4),
                    ("C_full+EWC", "full",   5e-4, 0.90, 1.0, 0.30, 8.0, 1e-4),
                ]
            elif shot <= 80:
                candidates = [
                    ("A_top+EWC", "top",     8e-4, 0.90, 1.0, 0.20, 3.0, 1e-5),
                    ("B_mid+EWC", "mid",     5e-4, 0.90, 1.0, 0.15, 2.0, 1e-5),
                    ("C_full+EWC", "full",   3e-4, 0.90, 0.5, 0.10, 1.0, 1e-6),
                ]
            else:
                candidates = [
                    ("A_mid+EWC", "mid",     5e-4, 0.90, 0.5, 0.10, 1.0, 1e-6),
                    ("B_full+EWC", "full",   3e-4, 0.90, 0.5, 0.08, 0.8, 1e-6),
                ]
            ...
            for cname, level, lr, tau, lam_u, lam_src, lam_ewc, lam_l2 in candidates:
                ...
                if dev_f1_cand > best["dev_f1"] + 1e-6:
                    best.update({
                        "name": cname,
                        "dev_f1": dev_f1_cand,
                        "thr": thr_cand,
                    ...
                    # free the models if not best to reduce memory (optional)
                    del student
                    del teacher

            _log(f"[DAEWC-DOMINANT] picked={best['name']} dev_f1={best['dev_f1']:.4f} thr={best['thr']:.3f} trainable={best['trainable_params']}")
