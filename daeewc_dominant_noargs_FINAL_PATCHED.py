...
def train_daeewc_fixmatch_plus(
    ...
    *,
    ...
    # --- constants (avoid dtype surprises) ---
    ...
    def train_step(x_l, y_l, x_u, x_s, y_s, lambda_u_t):
        ...
        with tf.GradientTape() as tape:
            ...
            ewc_pen = _ewc_penalty()
            l2_pen = _l2_anchor_penalty()
        ...
        # ---- EMA update teacher IN-PLACE (no python list, no out-of-scope) ----
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            if it_s is not None:
                ...
                # dummy (won't matter if lambda_src=0), but keeps signature stable
                x_s, y_s = x_l, y_l
        ...
        dev_f1 = float(m["macro_f1"])   # <-- FIX: macro_f1 key
